...
public class CheckDuplicatesInJavaArray {
	...
	/**
	 * brute force way of checking if array contains duplicates in Java comparing each elements to all other elements of array complexity on
	...
	/**
	 * detect duplicate in array by comparing size of List and Set since Set doesn't contain duplicate, size must be less for an array which
	 * contains duplicates
	 */
	...
	/**
	 * Since Set doesn't allow duplicates add() return false if we try to add duplicates into Set and this property can be used to check if
	 * array contains duplicates in Java
	 */
